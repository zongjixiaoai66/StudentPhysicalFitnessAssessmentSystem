<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.ceshishuju" label="测试数据" prop="ceshishuju">
          <file-upload
          tip="点击上传测试数据"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.ceshishuju?ruleForm.ceshishuju:''"
          @change="ceshishujuUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.ceshishuju" label="测试数据" prop="ceshishuju">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.ceshishuju)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.ceshididian = true;
          ...
          if(o=='ceshishuju'){
            this.ruleForm.ceshishuju = obj[o];
	    this.ro.ceshishuju = true;
            continue;
          }
          if(o=='xuehao'){
	...
	if(this.ruleForm.ceshishuju!=null) {
		this.ruleForm.ceshishuju = this.ruleForm.ceshishuju.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `ticeshuju/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.ticeshujuCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    ceshishujuUploadChange(fileUrls) {
	this.ruleForm.ceshishuju = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
