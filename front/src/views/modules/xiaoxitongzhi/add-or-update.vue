...
export default {
  ...
  methods: {
	...
	this.ruleForm.tongzhineirong = this.ruleForm.tongzhineirong.replace(reg,'../../../springboote52j4/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xiaoxitongzhi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xiaoxitongzhiCrossAddOrUpdateFlag = false;
						 this.parent.search();
